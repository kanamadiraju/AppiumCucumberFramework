...
public class AmazonStepDefinitions extends desiredCapabilities{
	...
	public void login_to_amazon_app() throws InterruptedException, MalformedURLException {
		

		Log.info("Waiting for the Element" + Amazon_HamburgerMenu_OR.HelloSignIn);
		wait.until(ExpectedConditions.elementToBeClickable(Amazon_HamburgerMenu_OR.HelloSignIn));
		if(!Amazon_HamburgerMenu_OR.HelloSignIn.getText().equalsIgnoreCase("Hello. Sign In")) 
		{
				CF.scrollToText("Settings", driver);
				Amazon_LoginPage_OR.Settings.click();
					
				Amazon_LoginPage_OR.SignOut.click();
				Amazon_LoginPage_OR.SignOutpopup.click();
				Log.info("Waiting for the Element" + Amazon_HomePage_OR.SkipSignIn);
				wait.until(ExpectedConditions.elementToBeClickable(Amazon_HomePage_OR.SkipSignIn));
				Log.info("Click the Element " + Amazon_HomePage_OR.SkipSignIn);
				Amazon_HomePage_OR.SkipSignIn.click();
	...
	WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//android.widget.EditText[@index='1']")));
				//elementToBeClickable((Amazon_HamburgerMenu_OR.MobileNumberOrEmail)));
	...
	public void choose_the_current_location() {
		...
		/* If "Location allow while using the app" option is disabled for amazon app permission setting...then below code need to uncomment 
		Log.info("Waiting for the Element" + Amazon_ProductPage_OR.AllowAccess);
		wait.until(ExpectedConditions.elementToBeClickable(Amazon_ProductPage_OR.AllowAccess));
		Log.info("Click the Element " + Amazon_ProductPage_OR.AllowAccess);
		Amazon_ProductPage_OR.AllowAccess.click();
	...
	*/	
	...
	public void verify_product_details() {
		...
		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOf(Amazon_ProductPage_OR.ProdcutName));
	...
	public void user_verifies_the_details() throws MalformedURLException, InterruptedException {
		...
		System.out.println("The Price of the Choosen Product is: "+ProductPrice);
...
//		Assert.assertTrue(Amazon_OR.FromTheManufacturer.isDisplayed());
	...
	//	String CartSuccessMessage = Amazon_ProductPage_OR.AddedToCart.getText();
	//	System.out.println(CartSuccessMessage);
	//	Assert.assertTrue(Amazon_ProductPage_OR.CartSuccess.isDisplayed());
	}
	...
	public void verify_Cart() {
		...
		String ProductCartPage = Amazon_ProductPage_OR.ProdcutNameInCart.getText();
		Log.info("Verify the Expected Element is displayd on Current Page");
		Assert.assertTrue(Amazon_ProductPage_OR.ProdcutNameInCart.isDisplayed());
		Log.info("Verify the Product is TV or not");
		Assert.assertTrue("The product is not present in the Cart", ProductCartPage.contains(data.Brand));
	}
